...
static struct minmax leocc_g_bw;
static u64 leocc_g_cycle_mstamp;
static u64 leocc_g_ack_epoch_mstamp;
static u16 leocc_g_extra_acked[2];
...
struct leocc {
	...
	u32 mode:3,		 
		...
		p_post_bw:5,
		p_post_rtt:5,
		local_reconfiguration_trigger:1;
...
static u16 leocc_extra_acked(const struct sock *sk)
{
	...
	return max(leocc_g_extra_acked[0], leocc_g_extra_acked[1]);
}
...
__bpf_kfunc static void leocc_cwnd_event(struct sock *sk, enum tcp_ca_event event)
{
	...
	if (event == CA_EVENT_TX_START && tp->app_limited) {
		...
		leocc_g_ack_epoch_mstamp = tp->tcp_mstamp;
				    ...
				    const struct rate_sample *rs)
{
	...
	struct leocc *leocc = inet_csk_ca(sk);
	bool is_full_length =
		tcp_stamp_us_delta(tp->delivered_mstamp, leocc_g_cycle_mstamp) >
		leocc->min_rtt_us;
	u32 inflight, bw;
...
static void leocc_advance_cycle_phase(struct sock *sk)
{
	...
	leocc_g_cycle_mstamp = tp->delivered_mstamp;
}
...
{
	struct leocc *leocc = inet_csk_ca(sk);
	if (!leocc_full_bw_reached(sk)  || leocc->local_reconfiguration_trigger)
...
static void leocc_update_bw(struct sock *sk, const struct rate_sample *rs)
{
	...
	if (!rs->is_app_limited || bw >= leocc_max_bw(sk)) {
		minmax_running_max(&leocc_g_bw, leocc_bw_rtts, leocc->rtt_cnt, bw);
	}
	leocc->latest_bw = bw;
}
				       ...
				       const struct rate_sample *rs)
{
	...
	if (leocc->round_start) {
		...
		if (leocc->extra_acked_win_rtts >= leocc_extra_acked_win_rtts) {
			...
			leocc->extra_acked_win_idx = leocc->extra_acked_win_idx ?
						   0 : 1;
			leocc_g_extra_acked[leocc->extra_acked_win_idx] = 0;
		}
	}

	epoch_us = tcp_stamp_us_delta(tp->delivered_mstamp,
				      leocc_g_ack_epoch_mstamp);
	...
	if (leocc->ack_epoch_acked <= expected_acked ||
		...
		leocc->ack_epoch_acked = 0;
		leocc_g_ack_epoch_mstamp = tp->delivered_mstamp;
		expected_acked = 0;
	}
	...
	if (extra_acked > leocc_g_extra_acked[leocc->extra_acked_win_idx])
		leocc_g_extra_acked[leocc->extra_acked_win_idx] = extra_acked;
}
...
static void leocc_check_drain(struct sock *sk, const struct rate_sample *rs)
{
	...
	if (leocc->mode == LEOCC_STARTUP && leocc_full_bw_reached(sk)) {
        ...
        leocc->local_reconfiguration_trigger = 0;
...
static void leocc_check_probe_rtt_done(struct sock *sk)
{
	...
	if (leocc->local_reconfiguration_trigger) {
		minmax_reset(&leocc_g_bw, leocc->rtt_cnt, leocc->reconfiguration_max_bw);
	}
}
...
static void leocc_update_min_rtt(struct sock *sk, const struct rate_sample *rs)
{
	...
	if (!leocc->idle_restart && leocc->mode == LEOCC_DYNAMIC_CRUISE && 
		((leocc_probe_rtt_mode_ms > 0 && filter_expired) || leocc->local_reconfiguration_trigger)) {
...
__bpf_kfunc static void leocc_main(struct sock *sk, const struct rate_sample *rs)
{
	...
	if (min_rtt_fluctuation > 0 && leocc->rtt_hat_post >= delta_rtt + leocc->min_rtt_us + min_rtt_fluctuation && !leocc->local_reconfiguration_trigger && leocc->mode == LEOCC_DYNAMIC_CRUISE)
	{
...
__bpf_kfunc static void leocc_init(struct sock *sk)
{
	...
	minmax_reset(&leocc_g_bw, leocc->rtt_cnt, 0);
	leocc_g_cycle_mstamp = 0;
	leocc_g_ack_epoch_mstamp = tp->tcp_mstamp;
	leocc_g_extra_acked[0] = 0;
	leocc_g_extra_acked[1] = 0;
