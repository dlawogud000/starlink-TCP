...
# # ping
# setsid bash "$BASE_DIR/bin/run_ping.sh" "$OUT_DIR" > "$OUT_DIR/ping_stdout.log" 2>&1 &
...
echo "[INFO] POP IP: $POP_IP" > "$OUT_DIR/pop_ip.txt"
...
# POP ping interval
setsid bash "$BASE_DIR/bin/pop_interval.sh" \
  "$POP_IP" \
