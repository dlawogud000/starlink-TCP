...
if [ "$PROTOCOL" = "tcp" ]; then
  iperf3 -B "$LOCAL_IP" -c "$SERVER_IP" -i "$IPERF_INTERVAL" -p "$SERVER_PORT" $REVERSE_FLAG -t "$DURATION" -i 1 --json \
    > "$OUT_DIR/iperf.json" 2> "$OUT_DIR/iperf.stderr.log"
elif [ "$PROTOCOL" = "udp" ]; then
  UDP_RATE="${UDP_RATE:-100M}"
  iperf3 -B "$LOCAL_IP" -c "$SERVER_IP" -i "$IPERF_INTERVAL" -p "$SERVER_PORT" -u $REVERSE_FLAG -b "$UDP_RATE" -t "$DURATION" -i 1 --json \
    > "$OUT_DIR/iperf.json" 2> "$OUT_DIR/iperf.stderr.log"
else
